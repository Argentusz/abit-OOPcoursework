    ...
    route ("/example") {
        route ("/v$apiV") {
            get() {
                ...
                val applies = mutableListOf(course.id())
                ...
                val nowy = course.requiredExams().toMutableList()
                nowy[0] = Exams.Physics
                call.respondText(
                    s.getScore(course.requiredExams()).toString() + course.requiredExams().toString() + nowy.toString()
                )
            }
        }
    }
